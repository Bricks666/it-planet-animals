...
func (ac *AreaCreate) SetID(u uint64) *AreaCreate {
	ac.mutation.SetID(u)
	return ac
}

// AddPointIDs adds the "points" edge to the Location entity by IDs.
func (ac *AreaCreate) AddPointIDs(ids ...uint64) *AreaCreate {
	ac.mutation.AddPointIDs(ids...)
	return ac
}

// AddPoints adds the "points" edges to the Location entity.
func (ac *AreaCreate) AddPoints(l ...*Location) *AreaCreate {
	ids := make([]uint64, len(l))
	for i := range l {
		ids[i] = l[i].ID
	}
	return ac.AddPointIDs(ids...)
}
...
func (ac *AreaCreate) check() error {
	...
	if v, ok := ac.mutation.ID(); ok {
		if err := area.IDValidator(v); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`ent: validator failed for field "Area.id": %w`, err)}
		}
	}
	return nil
}
...
func (ac *AreaCreate) createSpec() (*Area, *sqlgraph.CreateSpec) {
	...
	if nodes := ac.mutation.PointsIDs(); len(nodes) > 0 {
