...
func (aq *AreaQuery) QueryPoints() *LocationQuery {
...
func (aq *AreaQuery) FirstID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (aq *AreaQuery) FirstIDX(ctx context.Context) uint64 {
...
func (aq *AreaQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (aq *AreaQuery) OnlyIDX(ctx context.Context) uint64 {
...
func (aq *AreaQuery) IDs(ctx context.Context) (ids []uint64, err error) {
...
func (aq *AreaQuery) IDsX(ctx context.Context) []uint64 {
	ids, err := aq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (aq *AreaQuery) Clone() *AreaQuery {
	...
	return &AreaQuery{
		...
		withPoints: aq.withPoints.Clone(),
...
func (aq *AreaQuery) WithPoints(opts ...func(*LocationQuery)) *AreaQuery {
	query := (&LocationClient{config: aq.config}).Query()
...
func (aq *AreaQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Area, error) {
	...
	if query := aq.withPoints; query != nil {
		if err := aq.loadPoints(ctx, query, nodes,
			func(n *Area) { n.Edges.Points = []*Location{} },
			func(n *Area, e *Location) { n.Edges.Points = append(n.Edges.Points, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AreaQuery) loadPoints(ctx context.Context, query *LocationQuery, nodes []*Area, init func(*Area), assign func(*Area, *Location)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uint64]*Area)
	nids := make(map[uint64]map[*Area]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(area.PointsTable)
		s.Join(joinT).On(s.C(location.FieldID), joinT.C(area.PointsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(area.PointsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(area.PointsPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "points" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
