...
func (uq *UserQuery) QueryAnimals() *AnimalQuery {
	query := (&AnimalClient{config: uq.config}).Query()
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withAnimals: uq.withAnimals.Clone(),
...
func (uq *UserQuery) WithAnimals(opts ...func(*AnimalQuery)) *UserQuery {
	query := (&AnimalClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withAnimals = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withAnimals; query != nil {
		if err := uq.loadAnimals(ctx, query, nodes,
			func(n *User) { n.Edges.Animals = []*Animal{} },
			func(n *User, e *Animal) { n.Edges.Animals = append(n.Edges.Animals, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadAnimals(ctx context.Context, query *AnimalQuery, nodes []*User, init func(*User), assign func(*User, *Animal)) error {
	...
	query.Where(predicate.Animal(func(s *sql.Selector) {
		s.Where(sql.InValues(user.AnimalsColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "chipper_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
